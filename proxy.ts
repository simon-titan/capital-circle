...
export async function proxy(request: NextRequest) {
  ...
  const { data: rawProfile } = await supabase
    .from("profiles")
    .select(
      "codex_accepted,usage_agreement_accepted,is_admin,is_paid,application_status,membership_tier,access_until,last_login_at,churn_email_1_sent_at,churn_email_2_sent_at",
    )
    .eq("id", user.id)
    .single();
