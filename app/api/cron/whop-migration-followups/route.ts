...
import { sendWhopMigrationMail2, sendWhopMigrationMail3 } from "@/lib/email/templates";
...
/**
 * Cron: Whop-Migrations-Folgemails (Mail 2 + Mail 3).
 *
 ...
 * `POST /api/admin/campaigns/whop-migration`. Dieser Cron verarbeitet
...
export async function GET(request: NextRequest) {
  ...
  for (const [email, entry] of byEmail) {
    ...
    if (!entry.step1 && !entry.step2 && !entry.step0.opened_at) {
      if (daysSince(entry.step0.sent_at) >= WHOP_MIGRATION.mail2DueDays) {
        try {
          const firstName = await firstNameFor(email, entry.step0.user_id, profileNames);
          await sendWhopMigrationMail2({ firstName, email, userId: entry.step0.user_id });
          mail2Sent += 1;
        } catch (err) {
          const msg = err instanceof Error ? err.message : String(err);
    ...
    if (!entry.step2 && daysSince(entry.step0.sent_at) >= WHOP_MIGRATION.mail3DueDays) {
      ...
      if (opened && !clicked) {
        try {
          const firstName = await firstNameFor(email, entry.step0.user_id, profileNames);
          await sendWhopMigrationMail3({ firstName, email, userId: entry.step0.user_id });
          mail3Sent += 1;
        } catch (err) {
          const msg = err instanceof Error ? err.message : String(err);
