...
/**
 * POST /api/admin/step2-applications/[id]/reject
 *
 * Body: { reason?: string }
 *
 * Optimistic-Lock auf `status='pending'`.
...
export async function POST(
  ...
  if (updated.user_id) {
    const { error: profileErr } = await service
      .from("profiles")
      .update({ step2_application_status: "rejected" })
      .eq("id", updated.user_id as string);
    if (profileErr) {
      console.error("[admin/step2-applications/reject] profile sync failed:", profileErr);
    }
  }
