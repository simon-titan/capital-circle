...
- **E-Mails** (`lib/email/layout/styles.ts`, `components.tsx`, `BaseEmail.tsx`): eigene, bewusst schlichtere Fassung von v3.2 — **seit 20.09.2026 auf ausdrücklichen Nutzerwunsch „so schlicht wie möglich“.** Mail-Clients kennen keine CSS-Variablen, daher stehen die `--cc-*`-Werte in `styles.ts` als Hex, Linien als vorgerechnete Volltöne.
  - **Aufbau, für jede Mail gleich:** Wortmarke (Bild, 176 px breit, `${appUrl}/logo/cc-wortmarke-weiss.png` — in einer Mail muss jedes Bild absolut verlinkt sein), **eine** Graphit-Karte auf Graphit-Grund mit grauer Haarlinie, darin eine Überschrift, kurzer Text, **höchstens ein** Knopf; darunter die Fußzeile (Impressum · Datenschutz, bei Werbung Abmelden samt Widerspruchshinweis nach § 7 Abs. 3 Nr. 4 UWG).
  - **Nicht zurückholen:** Gold-Verlauf und Glow am Knopf, Champagner-Haarlinien und -Lichtkanten, Panel-Kästen als Hervorhebung, Versal-Labels über der Überschrift, zwei Trennlinien übereinander. Gold steht nur noch am Knopf und am Link, Rot nur als Störungsmeldung in den Betriebsmails.
  - **Eine Quelle:** Farben, Abstände und Schriftgrößen ausschließlich aus `EMAIL_TOKENS`; jede Vorlage benutzt die Bausteine aus `components.tsx` (`EmailHeading`, `EmailSubheading`, `EmailText`, `EmailSmall`, `EmailError`, `EmailButton`, `EmailLink`, `EmailRows`, `EmailQuote`, `EmailDivider`) und bringt **keinen** eigenen `style` mit. Die Migrations-Mails benutzen dieselben Bausteine; den Sonderbausatz `lib/email/campaigns/whop-migration/` gibt es nicht mehr.
  - **Technik:** Tabellen mit Inline-Stilen, Karte 560 px breit, Seitenrand auf der **Zelle** statt auf der Tabelle (`width="100%"` plus `padding` an derselben Tabelle ergibt Fensterbreite + 32 px), Flächen zusätzlich als `bgcolor`-Attribut, `color-scheme: dark`. Keine Webfonts: Inter, falls installiert, sonst System-Sans; Monospace nur für Zugangsdaten zum Abtippen.
...
- v3.2.1 Nachtrag (2026-09-20): **E-Mails schlicht.** Auf Nutzerwunsch („so schlicht wie möglich“) tragen alle 32 Vorlagen dasselbe kurze Gerüst: neue Wortmarke als Bild, eine Karte, eine Überschrift, höchstens ein Knopf, Fußzeile. Raus sind Gold-Verlauf und Glow am Knopf, die Champagner-Lichtkante unter der Wortmarke, `EmailHighlight`/`EmailEyebrow`/`EmailCard` und der Kampagnen-Bausatz `lib/email/campaigns/whop-migration/`. Neu sind `EmailRows` (Beschriftung/Wert), `EmailQuote` (fremder Text) und `EmailError` (Störung in den Betriebsmails). Das ist **keine** Änderung an der App: Glas, Gold-Kante und Glow gelten dort unverändert weiter — nur die Mails sind leiser.
